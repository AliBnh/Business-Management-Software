...
const TABLE_HEAD = ["Name", "Product Count", "Products List", "Action"];
...
export function Categories() {
  ...
  const handleSearch = async (e) => {
    ...
    if (searchTerm !== "") {
      setLoading(true);
      try {
        const response = await CategoryService.searchCategory(searchTerm);
        setCategories(response.data);
      } catch (error) {
        console.log(error);
      }
    ...
    <Card className="h-full w-full border-t border-blue-gray-5 ">
      <CardHeader floated={false} shadow={false} className="rounded-none ">
        <div className="mb-8 flex items-center justify-between gap-8">
          <div>
            <Typography variant="h5" className="text-left text-black">
              Categories list
            </Typography>
